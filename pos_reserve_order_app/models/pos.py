...
class POSOrderInherit(models.Model):
    ...
    delivery_date = fields.Date(string='Reserve Order delivery Date',
                                readonly=True)
    ...
    def unlink(self):
        for pos_order in self.filtered(
                lambda pos_order: pos_order.state in ['reserved']):
            pos_order.write({'state': 'cancel'})

        for pos_order in self.filtered(
                lambda pos_order: pos_order.state not in ['draft', 'cancel']):
            raise UserError(_(
                'In order to delete a sale, it must be new or Reserved or cancelled.'))
        ...
        for order in orders:
            ...
            if 'server_id' in order['data']:
                existing_order = self.env['pos.order'].search(
                    [('id', '=', order['data']['server_id'])], limit=1)
            process_orders = self._process_order(order, draft, existing_order)
            order_ids = process_orders.ids
    ...
    def update_stock_qty(self, line):
        if line.product_id.type == 'product':
            ...
            config_loc = line.order_id.location_id.id
            reserved_loc = line.order_id.config_id.reservation_location.id
            config_loc_stock = stock_quant.search(
                [('product_id', '=', line.product_id.id),
                 ('location_id', '=', config_loc)])
            reserved_loc_stock = stock_quant.search(
                [('product_id', '=', line.product_id.id),
            ...
            if config_loc_stock:
                for stock in config_loc_stock:
                    ...
                    config_loc_stock.write({
                        'quantity': config_loc_qty + abs(
                            line.old_qty - line.qty) if line.old_qty > line.qty else config_loc_qty - abs(
                            line.old_qty - line.qty)
                    })
            else:
                updt_config_qty = {
                    'product_id': line.product_id.id,
            ...
            if reserved_loc_stock:
                for reserve in reserved_loc_stock:
                    ...
                    reserved_loc_stock.write({
                        'quantity': reserved_loc_qty - abs(
                            line.old_qty - line.qty) if line.old_qty > line.qty else reserved_loc_qty + abs(
                            line.old_qty - line.qty),
                    })
            else:
                updt_reserve_qty = {
    ...
    def change_or_remove_product(self, cancelorder_products, removedline,
                                 config_id, is_del_all):
        lines = []
        config = self.env['pos.config'].search([('id', '=', int(config_id))])
        if is_del_all:
            if config.cancel_charge_type == 'percentage':
                temp_charge = (
                                          self.amount_total * config.cancel_charges) / 100.0;
            else:
            ...
            if removedline:
                for line in removedline:
                    rline = self.env['pos.order.line'].search(
                        [('id', '=', int(line))])
                    if config.cancel_charge_type == 'percentage':
                        temp_charge = (
                                                  rline.price_subtotal_incl * config.cancel_charges) / 100.0;
                    else:
                        temp_charge = config.cancel_charges;
                ...
                for k, v in cancelorder_products.items():
                    pos_line = self.env['pos.order.line'].search(
                        [('id', '=', int(k))])
                    fpos = pos_line.order_id.fiscal_position_id
                    tax_ids_after_fiscal_position = fpos.map_tax(
                        pos_line.tax_ids, pos_line.product_id,
                        pos_line.order_id.partner_id) if fpos else pos_line.tax_ids
                    price = pos_line.price_unit * (
                                1 - (pos_line.discount or 0.0) / 100.0)
                    taxes = tax_ids_after_fiscal_position.compute_all(price,
                                                                      ...
                                                                      partner=pos_line.order_id.partner_id)
                    if pos_line.qty > v:
                        if config.cancel_charge_type == 'percentage':
                            diff_price = (
                                                     pos_line.qty - v) * pos_line.price_unit
                            temp_charge = (
                                                      diff_price * config.cancel_charges) / 100.0;
                        else:
                            temp_charge = config.cancel_charges;
                        vals = {
                            'discount': 0,
        ...
        amount_tax = currency.round(sum(
            self._amount_line_tax(line, self.fiscal_position_id) for line in
            self.lines))
        amount_untaxed = currency.round(
            sum(line.price_subtotal for line in self.lines))
    ...
    def get_reserved_lines(self):
        reserved_orders = self.env['pos.order'].search(
            [('state', '=', 'reserved')])
        print(
            "reserved_orders=============================================================",
            reserved_orders)
    ...
    def pay_reserved_amount(self, select_journal_id, pay_amount, cash_jrnl_id,
                            session_id):
        ...
        if pay_amount > self.amount_due:
            ...
            self.write({
                'state': 'paid',
                'amount_paid': sum(
                    payment.amount for payment in self.payment_ids),
                'amount_due': 0.0,
            })
            for line in self.lines:
