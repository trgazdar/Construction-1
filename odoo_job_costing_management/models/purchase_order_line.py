...
class PurchaseOrderLine(models.Model):
    ...
    def _prepare_stock_move_vals(self, picking, price_unit, product_uom_qty, product_uom):
        self.ensure_one()
        result = super(PurchaseOrderLine, self)._prepare_stock_move_vals(picking=picking, price_unit=price_unit, product_uom_qty=product_uom_qty, product_uom=product_uom)
        result.update({'cost_id': self.job_cost_id and self.job_cost_id.id, 'job_cost_line_id': self.job_cost_line_id and self.job_cost_line_id.id})
        return result
#
#
