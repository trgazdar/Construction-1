...
#             rec.actual_purchase_cost = sum([p.order_id.state in ['purchase', 'done'] and (p.product_qty * p.price_unit) for p in rec.purchase_order_line_ids])
    ...
    # @api.depends('account_invoice_line_ids', 'account_invoice_line_ids.quantity','account_invoice_line_ids.price_unit','account_invoice_line_ids.total_amount','account_invoice_line_ids.move_id.state')
    # def _compute_actual_vendor_cost(self):
        # for rec in self:
            ...
            # rec.actual_vendor_cost = actual_vendor_cost_v
            # rec.actual_invoice_quantity = sum([p.total_amount for p in rec.account_invoice_line_ids])
