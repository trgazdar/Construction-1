...
class CVISubmittal(models.Model):
    _name = 'cvi.submittal'
    ...
    code = fields.Selection(
        ...
        default='e')
    cviID = fields.Char(string="CVI ID", required=False, )
    ...
    cvi_seq_num = fields.Integer(string="", required=False, )
    trans_mt_seq = fields.Integer(string="", )
    trans_dc_seq = fields.Integer(string="", )
    trans_wd_seq = fields.Integer(string="", )
    parent_cvi = fields.Many2one(comodel_name="cvi.submittal", string="", required=False, )
    ...
    def action_dc_send(self):
        ...
        try:
            template_id = ir_model_data.get_object_reference("wc_document_control", 'email_template_cvi_submittal')[1]
        except ValueError:
            template_id = False
        try:
    ...
    def get_revisions_num(self):
        self.revisions_num = self.env['cvi.submittal'].search_count([('parent_cvi', '=', self.id)])
    ...
    def resbmittal_button(self):
        cvi = self.env['cvi.submittal']
        revision_no = []
        if int(self.revision_no_seq) == 0:
            current_revision_num = 1
        else:
            for record in self.env['cvi.submittal'].search([('parent_cvi', '=', self.parent_cvi.id)]):
        ...
        cvi.create({
            ...
            'cviID': self.cviID,
            'revision_no': str(self.cviID) + '/REV' + str(current_revision_num),
            'revision_no_seq': str(current_revision_num),
            'parent_cvi': self.id if self.revision_no_seq == 0 else self.parent_cvi.id,
        })
    ...
    def create(self, values):
        # Add code here
        res = super(CVISubmittal, self).create(values)
        if not res.cviID:
            prev_trans_mt_num = []
            prev_trans_dc_num = []
            prev_trans_wd_num = []
            for rec in self.env['cvi.submittal'].search([('project_id', '=', res.project_id.id)]):
                if rec != False:
            ...
            if res.submittal_type == 'mt':
                ...
                res.cviID = seq_const + str('CVIMT-') + str(res.trans_mt_seq)
            if res.submittal_type == 'dc':
                res.trans_dc_seq = max(prev_trans_dc_num) + 1 if prev_trans_dc_num else 1
                res.cviID = seq_const + str('CVIDC-') + str(res.trans_dc_seq)
            if res.submittal_type == 'dw':
                res.trans_wd_seq = max(prev_trans_wd_num) + 1 if prev_trans_wd_num else 1
                res.cviID = seq_const + str('CVIDW-') + str(res.trans_wd_seq)
        ...
        if not res.revision_no:
            ...
            res.revision_no = str(res.cviID) + '/REV' + str(res.revision_no_seq)
    ...
    def open_related_revisions(self):
        return {
            'type': 'ir.actions.act_window',
            'res_model': 'cvi.submittal',
            'view_mode': 'tree,form',
            'view_type': 'form',
            'name': 'Revisions',
            'target': 'current',
            'domain': [('parent_cvi', '=', self.id)],
